      ...
      case CNewMenu::STYLE_ICY_NOBORDER:
        {
          ...
          for (int n=rect.top+m_cyTopBorder+2*CY_BORDER_GRIPPER;n<rect.Height()-m_cyTopBorder-m_cyBottomBorder-4*CY_BORDER_GRIPPER;n+=4)
          {
           ...
           // make round corners
// sakamoto îwåiêFÇïœçX
          color = GetSysColor(COLOR_MENU);//GetSysColor(COLOR_3DLIGHT);//CNewMenu::GetMenuBarColor();
          CRect temp(rect);
...
BOOL CNewToolBar::OnEraseBkgnd(CDC* pDC)
{
  ...
  default:
    {
// sakamoto îwåiêFÇïœçX
      pDC->FillSolidRect(rect,GetSysColor(COLOR_MENU/*COLOR_3DLIGHT*/));
    }
    break;
  }
  return bRet;
...
// DSS_MONO Ç DSS_DISABLED Ç…ïœçXÅBâeÇîñÇ≠
      pDC->DrawState(ptImage, imgSize, hDrawIcon2, DSS_DISABLED, (HBRUSH)NULL);
...
break;
  ...
  else
  {
    ...
    case CNewMenu::STYLE_ICY_NOBORDER:
    default:
// sakamoto îwåiéÆÇïœçX
      tempBrush.CreateSolidBrush(GetSysColor(COLOR_MENU)/*GetSysColor(COLOR_3DLIGHT)*/);
      break;
    }
